...
//Page 2 - Application :: start
	...
	function sendValidationCode(){
		...
		jQuery.ajax({
			url: 'http://stage.libertylendingexchange.com/application/sendValidationCode',
			method:'post',
			data:sender,
			headers:{
	...
	function verifyValidationCode(){
		jQuery.ajax({
			url: 'http://stage.libertylendingexchange.com/application/verifyValidationCode/'+jQuery('#ValidateEmail').val(),
			method:'POST',
			async:false,
			headers:{
				'x-keyStone-nonce': nonce
